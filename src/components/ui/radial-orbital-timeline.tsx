...
interface TimelineItem {
  ...
  energy: number;
  /** Optional URL — if provided, shows an "Enter Module" button on the card */
  href?: string;
}

interface RadialOrbitalTimelineProps {
  timelineData: TimelineItem[];
  /** Called when the user clicks "Enter Module" on an expanded node */
  onNodeNavigate?: (href: string) => void;
}
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-4xl h-full flex items-center justify-center">
        ...
        >
          {/* Central nucleus */}
          <div className="absolute w-16 h-16 rounded-full bg-gradient-to-br from-[#5046e4] to-[#1e00a5] animate-pulse flex items-center justify-center z-10">
            ...
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#3c3550] via-[#5046e4] to-[#c3c0ff]" />
          </div>
          ...
          {timelineData.map((item, index) => {
            ...
            return (
              ...
              >
                ...
                {/* Label */}
                <div className={`absolute top-24 whitespace-nowrap text-xs font-semibold tracking-wider transition-all duration-300 ${isExpanded ? "text-white scale-125" : "text-white/70"}`}>
                  {item.title}
                </div>

                {/* Expanded card */}
                {isExpanded && (
                  <Card className="absolute top-36 left-1/2 -translate-x-1/2 w-72 bg-black/90 backdrop-blur-lg border-white/30 shadow-xl shadow-white/10 overflow-visible">
                    ...
                    <CardContent className="text-xs text-white/80">
                      ...
                      {/* Navigate button */}
                      {item.href && onNodeNavigate && (
                        <button
                          className="mt-4 w-full flex items-center justify-center gap-2 py-2 rounded-lg bg-white/10 hover:bg-white text-white hover:text-black text-[10px] font-black uppercase tracking-[0.2em] transition-all duration-200 border border-white/20 hover:border-white cursor-pointer"
                          onClick={(e) => {
                            e.stopPropagation();
                            onNodeNavigate(item.href!);
                          }}
                        >
